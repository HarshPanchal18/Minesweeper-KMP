...
@Composable
fun loadImage(src: String) = painterResource(src)
...
fun Game(requestWindowSize: ((width: Dp, height: Dp) -> Unit)? = null) = MainLayout {
    ...
    Column(
        modifier = Modifier
            .background(color = Color(0xed, 0x9c, 0x38))
        ...
        Row {
            ...
            Column(modifier = Modifier.padding(8.dp)) {
                Box { Text(text = "New Game", fontSize = 20.sp) }
                Row {
        ...
        // Status
        Box(modifier = Modifier.padding(4.dp).size(200.dp, 32.dp)) { Text(message ?: "") }
...
@Composable
fun MainLayout(block: @Composable ColumnScope.() -> Unit) {
    Column { block() }
}
